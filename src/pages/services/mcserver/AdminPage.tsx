...
function AdminResouceCard({ name, id, description, state, type, icon, required, links, editId, deleteCard }: any) {
    ...
    <Card className="bg-dark text-white mb-3">
        <Card.Body>
            ...
            <p>Icon: {icon ?? "null"}</p>
            <p>Required</p>
            <ul>{required.map((value: number)=>{
                return <li key={value}>{value}</li>
            })}</ul>
            <p>Links</p>
            <ul>{links.map((value: any)=>{
...
export default function AdminPage(){
    ...
    const [editing,setEditing] = useState<any>({ name: "", icon: null, description: "", state: "admited", type: "datapack", links: [], required: [], images: []  });
    ...
    useEffect(()=>{
        if(!editId || !resources){
            setEditing({ name: "", icon: "https://media.forgecdn.net/avatars/377/691/637555914455642734.png", description: "", state: "admited", type: "datapack", links: [], required: [], images: [] });
            return;
        };
